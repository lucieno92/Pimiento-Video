...
class DropZone(QFrame):
    ...
    _HINT = "Drop a video or audio file here\n(or click \"Browse\" below)"
    ...
    def show_filename(self, path):
        """Affiche le nom du fichier chargé DANS la zone, en blanc."""
        import os as _os
...
class MediaInfoPage(QWidget):
    ...
    def _load_file(self, path):
        if not os.path.isfile(path):
            return
        # Afficher le nom du fichier dans la zone de dépôt (en blanc)
        if hasattr(self, "drop_zone"):
            self.drop_zone.show_filename(path)
        self._current_filename = os.path.basename(path)
        # Le label secondaire (sous la zone) reste vide : le nom est déjà
        # affiché dans la zone de dépôt elle-même.
