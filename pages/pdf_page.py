...
class DropList(QFrame):
    ...
    def __init__(self, label="Drop files here", parent=None):
        ...
        from PySide6.QtWidgets import QVBoxLayout, QSizePolicy as _SP
        l = QVBoxLayout(self)
        self._hint = label
    ...
    def show_files(self, paths):
        ...
        if len(paths) == 1:
            ...
            txt = f"{len(paths)} files selected"
...
class PdfPage(QWidget):
    ...
    def _single_file_tab(self, drop_text, attr, ext):
        ...
        drop.files_dropped.connect(lambda p: self._set_file(p[0], attr, lbl, ext, drop))
        l.addWidget(drop); l.addWidget(lbl)
        btn = QPushButton("Browse...")
        btn.clicked.connect(lambda: self._browse_single(attr, lbl, ext, drop))
        l.addWidget(btn)
        return w, l
    ...
    def _set_file(self, path, attr, lbl, ext, drop=None):
        if os.path.splitext(path)[1].lower() == ext:
            setattr(self, attr, path)
            if drop is not None:
                drop.show_files([path])
                lbl.setText("")
            else:
                lbl.setText(os.path.basename(path))

    def _browse_single(self, attr, lbl, ext, drop=None):
        path, _ = QFileDialog.getOpenFileName(self, "Select file", filter=f"Files (*{ext})")
        if path: self._set_file(path, attr, lbl, ext, drop)
    ...
    def _run_merge(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save merged PDF", os.path.join(_dod(), "merged.pdf"), "PDF (*.pdf)")
    ...
    def _run_to_word(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save Word file", os.path.join(_dod(), base+".docx"), "Word (*.docx)")
    ...
    def _run_to_excel(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save Excel file", os.path.join(_dod(), base+".xlsx"), "Excel (*.xlsx)")
    ...
    def _run_compress(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save compressed PDF", os.path.join(_dod(), base+"_compressed.pdf"), "PDF (*.pdf)")
    ...
    def _run_to_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save PDF", os.path.join(_dod(), base+".pdf"), "PDF (*.pdf)")
