...
class AudioDropZone(QFrame):
    ...
    _HINT = "Drop an audio or video file here\n(or click \"Browse\" below)"
    ...
    def __init__(self, parent=None):
        ...
        # Le label ne doit jamais imposer sa largeur au panneau : on autorise
        # sa réduction horizontale.
    ...
    def show_filename(self, path):
        ...
        # Tronquer au milieu si le nom est trop large pour la zone, afin de
        # ne jamais forcer l'élargissement du panneau.
...
class TranscriptionPage(QWidget):
    ...
    def _build_ui(self):
        ...
        # Empêcher un nom de fichier long d'élargir le panneau : largeur bornée
        # et texte tronqué avec "…" au milieu.
    ...
    def _load_file(self, path):
        ...
        # Afficher le nom DANS la zone de dépôt (en blanc), comme les autres
        # modules. Le petit label secondaire est vidé pour ne pas déformer
        # le panneau avec un nom long.
        if hasattr(self, "drop_zone"):
            self.drop_zone.show_filename(path)
        self.file_label.setText("")
        self.file_label.setToolTip(os.path.basename(path))
    ...
    def _export(self):
        ...
        _start = os.path.join(_dod(), f"transcript.{fmt.lower()}")
        path, _ = QFileDialog.getSaveFileName(self, "Exporter", _start, filters[fmt])
        if not path:
            return
        try:
            if fmt == "TXT":
