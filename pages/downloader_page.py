...
class DownloadWorker(QThread):
    ...
    def _try_update_ytdlp(self):
        """S'assure qu'un binaire yt-dlp EXTERNE et à jour est disponible.
        C'est ce binaire (stocké dans le dossier utilisateur) qui est utilisé
        pour télécharger : il se met à jour tout seul, donc YouTube et les
        autres sites continuent de fonctionner sans réinstaller l'app."""
        try:
            from core.ytdlp_manager import ensure_ytdlp
            self._ytdlp_bin = ensure_ytdlp(log=self.log_message.emit)
        except Exception:
            self._ytdlp_bin = None
    ...
    def run(self):
        ...
        # Tenter de mettre yt-dlp à jour (YouTube change souvent et casse les
        # anciennes versions). Non bloquant : si ça échoue, on continue avec
        # la version en place.
        self._try_update_ytdlp()
        ...
        for index, url in enumerate(self.urls, start=1):
            ...
            downloaded_ok = False
            # Priorité au binaire externe à jour (résout les blocages YouTube
            # sans réinstaller l'app). Repli sur l'API Python si indisponible.
            if getattr(self, "_ytdlp_bin", None):
                try:
                    downloaded_ok = self._download_with_binary(
                        url, per_url_opts, index, total, ffmpeg_location)
                    if downloaded_ok:
    ...
    def _try_with_cookies(self, url, base_opts, index, total, quiet=False):
        ...
        import sys
        import yt_dlp
        # Sur macOS, Chrome/Edge/Brave/Opera chiffrent leurs cookies via le
        # trousseau système ("Chrome Safe Storage"), ce qui ouvre une fenêtre
        # de mot de passe bloquante. On les évite sur Mac et on privilégie
        # Firefox (cookies non chiffrés par le trousseau).
        ...
        if not quiet:
            self.log_message.emit(
                ...
                f"cookies. Make sure you are logged in in Firefox, and that the "
                f"browser is closed during download.")
        return False

    def _download_with_binary(self, url, opts, index, total, ffmpeg_location):
        """Télécharge via le binaire yt-dlp externe (à jour). Traduit les
        options en arguments ligne de commande et parse la progression.
        Retourne True si le fichier est bien téléchargé."""
        import subprocess, re

        binpath = self._ytdlp_bin
        outtmpl = opts.get("outtmpl") or os.path.join(
            self.output_dir, "%(title)s.%(ext)s")
        fmt = opts.get("format") or "best"

        args = [binpath, "--newline", "--no-warnings",
                "-f", fmt, "-o", outtmpl]
        ...
        if self.mode == "video" and self.container:
            args += ["--merge-output-format", self.container]

        # Post-traitement audio (extraction) si mode audio
        for pp in (opts.get("postprocessors") or []):
            if pp.get("key") == "FFmpegExtractAudio":
                args += ["-x", "--audio-format",
                         pp.get("preferredcodec", "mp3")]

        # En-têtes navigateur (compatibilité TikTok/Facebook/Instagram)
        ...
        creationflags = 0
        if os.name == "nt":
            creationflags = 0x08000000  # pas de fenêtre console noire
        ...
        for line in proc.stdout:
            ...
            if m:
                try:
                    self.progress_value.emit(int(float(m.group(1))))
                except ValueError:
                    pass
            elif "Merging" in line or "Destination" in line:
                if not finished_merge:
                    finished_merge = True
                    self.log_message.emit("  -> Merging / post-processing…")
...
class DownloaderPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.output_path = QLineEdit(default_output_dir())
