"""Module 8: Audio Lab — noise reduction, voice enhance, vocal/music separation, AI voice-over."""
...
class AudioDropZone(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        l = QVBoxLayout(self)
        lbl = QLabel("Drop an audio or video file here\n(MP3, WAV, FLAC, MP4, MOV...)")
...
class AudioCleanPage(QWidget):
    ...
    def _make_drop_zone(self):
        ...
        box.addLayout(row)
        # Mémoriser tous les labels pour les tenir à jour ensemble
    ...
    def _load_file(self, path):
        if os.path.isfile(path):
            ...
            # Mettre à jour le label de chaque onglet qui a une drop zone
            for lbl in getattr(self, "_file_labels", []):
                lbl.setText(name)
                lbl.setStyleSheet("color:#e8542e;")
    ...
    def _run_denoise(self):
        ...
        out, _ = QFileDialog.getSaveFileName(self, "Save cleaned file", f"{base}_cleaned.{ext}", f"Audio (*.{ext})")
        if not out: return
    ...
    def _run_enhance(self):
        ...
        ext = EXPORT_FORMATS[fmt_key]["ext"]
        base = os.path.splitext(self.audio_path)[0]
        out, _ = QFileDialog.getSaveFileName(self, "Save enhanced file", f"{base}_enhanced.{ext}", f"Audio (*.{ext})")
        if not out: return
